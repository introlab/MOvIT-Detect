...
#define Millis() (std::chrono::high_resolution_clock::now())

// Record the current time to check an upcoming timeout against
#define StartTimeout() (timeout_start_ms = Millis())

// Check if timeout is enabled (set to nonzero value) and has expired
#define CheckTimeoutExpired() (io_timeout > 0 && (uint32_t)((std::chrono::duration_cast<std::chrono::milliseconds>(Millis() - timeout_start_ms)).count()) > io_timeout)
...
void VL53L0X::StartContinuous(uint16_t period_ms)
{
  WriteReg(0x80, 0x01);
  WriteReg(0xFF, 0x01);
  WriteReg(0x00, 0x00);
  ...
  if (period_ms != 0)
  {
    ...
    WriteReg16Bit(SYSTEM_INTERMEASUREMENT_PERIOD, period_ms);
